...
Task Build -Depends SetupContext, Clean, NugetRestore{
  ...
  Exec { msbuild "$($global:context.sln_file_info.FullName)" /t:ReBuild /p:Configuration=$($global:context.configuration) /v:$verboseLevel }
}


Task Test -Depends SetupContext {
  $xunit_consoleRunner = Join-Path $($global:context.sln_file_info.Directory.FullName) "\packages\xunit.runner.console.2.1.0\tools\xunit.console.exe"
  
  Get-ChildItem -Path $($global:context.root_dir) -Filter *Tests.dll -Recurse | ? {$_.FullName.Contains("\bin\$($context.configuration)") } |
  % {    
    Write-Verbose "$xunit_consoleRunner @($($_.FullName), '-quiet')" 
    & $xunit_consoleRunner @($_.FullName, '-quiet')
